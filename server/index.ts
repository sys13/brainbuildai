...
app.use((req, res, next) => {
	...
	if (proto === 'http') {
		...
		res.redirect(`https://www.${host}${req.originalUrl}`)
		return
	}
	next()
})
...
app.use((req, res, next) => {
	// Skip rate limiting entirely for e2e tests
...
async function getBuild() {
	try {
		...
		return { build: null as unknown as ServerBuild, error }
	}
}
...
const server = app.listen(portToUse, (err) => {
	if (err) {
