...
// @ts-expect-error - variables maybe undefined
const s3Client = new S3Client({
	region: STORAGE_REGION,
	endpoint: STORAGE_ENDPOINT, // Tigris endpoint
	credentials: {
		accessKeyId: STORAGE_ACCESS_KEY,
		secretAccessKey: STORAGE_SECRET_KEY,
	},
	forcePathStyle: true, // Important for Tigris/S3 compatible services
})
...
// 	console.log('process.env.AWS_REGION', url)
// 	const uploadResponse = await fetch(url, {
// 		method: 'PUT',
// 		headers: {
...
// 	})
// 	if (!uploadResponse.ok) {
// 		const errorMessage = `Failed to upload file. Status ${uploadResponse.status}: ${uploadResponse.statusText}`
...
export async function uploadDesignImage(userId: string, file: File) {
	const fileId = crypto.randomUUID()
	const fileExtension = file.name.split('.').pop() || ''
	const timestamp = Date.now()
	const key = `designs/${userId}/${timestamp}-${fileId}.${fileExtension}`

	await uploadToStorage(file, key)

	return key
}
export async function uploadContextPdf(userId: string, file: File) {
	const fileId = crypto.randomUUID()
	const fileExtension = file.name.split('.').pop() || ''
	const timestamp = Date.now()
	const key = `context/${userId}/${timestamp}-${fileId}.${fileExtension}`
...
function getBaseSignedRequestInfo({
	...
	if (!STORAGE_SECRET_KEY || !STORAGE_REGION) {
