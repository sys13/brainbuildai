	...
	(
		...
		const setOpen = React.useCallback(
			(value: ((value: boolean) => boolean) | boolean) => {
				...
				formData.append('sidebarOpen', String(!!open))
				fetcher.submit(formData, {
					method: 'post',
					action: '/resources/toggle-sidebar',
				})
			},
...
>(
	(
		{ asChild = false, className, isActive, onClick, size = 'md', ...props },
		ref,
	) => {
...
type SidebarToggleButtonProps = React.ComponentProps<'button'> & {
	className?: string
}
	...
	if (isMobile)
		return (
			<button
				...
				className={twMerge(
					'-m-2.5 p-2.5 pr-0 text-primary xl:hidden',
					className,
				)}
				{...props}
			>
